...
urlpatterns = [                                                 # <- multiLang:19) make standart urlpatterns list
    ...
    path('list/<str:model_name>:add/', views.Add_data, name='add_data'),        #!!!!!!!!!!<----- need slug instead "modelname" |because "modelname" in url not translated|
    path('list/<str:model_name>:<int:pk>/', views.Edit_data, name='edit_data'), #!!!!!!!!!!<----- need slug instead "modelname" |when lang is switching                   |
